...
class YourEventsReSelectServiceTest extends TestCase
{
    ...
    public function test_status_groups_venue_and_selected_addons_as_one_service(): void
    {
        Schema::dropIfExists('users');
        Schema::create('users', function (Blueprint $table) {
            $table->increments('user_id');
        ...
        $user = User::create([
            'username' => 'venue-addons-status-user',
            'full_name' => 'Venue Addons Client',
            'email' => 'venueaddons@test.com',
            'password' => Hash::make('password'),
            'role' => 'client',
            'status' => 'approved',
            'email_verified_at' => now(),
        ...
        $eventId = DB::table('events')->insertGetId([
            ...
            'title' => 'Wedding Event',
            'event_type' => 'Wedding',
            'venue_name' => 'La Tehillah Private Resort and Events Place',
            'venue_status' => 'pending confirmation',
            'catering' => 'La Tehillah Private Resort and Events Place',
            'catering_status' => 'pending confirmation',
            'host' => 'La Tehillah Private Resort and Events Place',
            'host_status' => 'pending confirmation',
            'created_at' => now(),
        ]);

        $response = $this->getJson("/your-events/{$eventId}/status");

        $response->assertOk();
        $response->assertJsonCount(1, 'services');
        $response->assertJsonPath('services.0.service_key', 'venue');
        $response->assertJsonPath('services.0.type', 'Venue + Add-ons');
        $response->assertStringContainsString('La Tehillah Private Resort and Events Place', $response->json('services.0.name'));
        $response->assertStringContainsString('Catering', $response->json('services.0.name'));
    }
}
