...
<div class="booking-modal" id="bookingConfirmModal" aria-hidden="true"><div class="booking-panel" role="dialog" aria-modal="true"><h2>Confirm Re-selection</h2><p>Are you sure you want to select this replacement service?</p><div class="booking-actions"><button class="secondary" type="button" data-close-booking>Cancel</button><button class="primary" type="button" data-confirm-booking>Yes, Select Service</button></div></div></div>
...
async function confirmReselect(serviceName, price) {
...
document.querySelector('[data-confirm-venue]').addEventListener('click', async () => {
    ...
    closeVenueModals();
    document.getElementById('bookingConfirmModal').querySelector('p').textContent = `Are you sure you want to select ${pendingVenue.name} as the replacement service?`;
    setBookingModal(bookingConfirmModal, true);
    bookingConfirmModal.dataset.name = pendingVenue.name;
    bookingConfirmModal.dataset.price = servicePrice || '';
});
document.querySelector('.select-service').addEventListener('click', () => {
    if (serviceKey === 'venue') {
        openVenueAvailability(serviceName, servicePrice);
        return;
    }
    document.getElementById('bookingConfirmModal').querySelector('p').textContent = `Are you sure you want to select ${serviceName} as the replacement service?`;
    bookingConfirmModal.dataset.name = serviceName;
    bookingConfirmModal.dataset.price = servicePrice || '';
    setBookingModal(bookingConfirmModal, true);
});
document.querySelector('[data-close-booking]').addEventListener('click', () => setBookingModal(bookingConfirmModal, false));
document.querySelector('[data-confirm-booking]').addEventListener('click', () => confirmReselect(bookingConfirmModal.dataset.name, Number(bookingConfirmModal.dataset.price) || servicePrice));
document.querySelector('[data-back-services]').addEventListener('click', () => doSelection(serviceKey, bookingConfirmModal.dataset.name, Number(bookingConfirmModal.dataset.price) || servicePrice));
</script>@endunless
</body></html>
