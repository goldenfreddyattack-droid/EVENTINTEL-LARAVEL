...
function getCateringImage($index) {
  $images = [
    'https://images.unsplash.com/photo-1528716321683-0fd0e0ea5f2f?auto=format&fit=crop&w=900&q=80',
...
function getCateringGallery($index) {
  $images = [
    'https://images.unsplash.com/photo-1516685018646-549c3bda0d3c?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1532634726-8b9fb99825f6?auto=format&fit=crop&w=900&q=80',
...
<body>
  ...
  <div class="container">
    ...
    <div class="hero">
      <div>
        <h1><?= $selectedService ? 'Catering Details' : 'Select Catering' ?></h1>
        <p><?= $selectedService ? 'Review this catering service before booking.' : 'Choose the ideal catering service for your event. Explore different cuisines, guest capacities, and premium menu packages.' ?></p>
      </div>

      <div class="search-box">
        <i class="fa-solid fa-magnifying-glass"></i>
        <input type="text" placeholder="Search cuisine or catering service...">
      </div>
    </div>
      ...
      <div class="service-detail">
        <div class="detail-card">
          <div class="detail-image">
            <img src="<?= esc(getCateringGallery($selectedIndex)) ?>" alt="<?= esc($selectedService['name']) ?>">
          </div>
        </div>
        <div class="detail-info">
          <div class="badge">Catering Service</div>
          <h1><?= esc($selectedService['name']) ?></h1>
          <p><?= esc($selectedService['description'] ?? 'Custom catering packages for your celebration.') ?></p>
          <div class="detail-stats">
            <div class="detail-stat">
              <span>Rating</span>
              <strong><?= number_format($selectedService['rating'] ?? 4.7, 1) ?> ★</strong>
            </div>
            <div class="detail-stat">
          ...
          <div class="detail-stats">
            <div class="detail-stat">
              <span>Supplier</span>
              <strong><?= esc($selectedService['supplier_name'] ?? 'Catering Pro') ?></strong>
            </div>
            <div class="detail-stat">
              <span>Menu Style</span>
              <strong><?= esc($selectedService['style'] ?? 'Buffet & plated') ?></strong>
            </div>
          </div>
          <button class="select-btn" onclick="selectService('<?= esc(addslashes($selectedService['name'])) ?>','catering')">Book Service</button>
          <div class="thumbnail-row">
            <?php for ($i = 0; $i < 4; $i++): ?>
              <div class="thumbnail">
                <img src="<?= esc(getCateringGallery($i)) ?>" alt="Thumbnail <?= $i + 1 ?>">
              </div>
            <?php endfor; ?>
          </div>
        </div>
    ...
    <form method="GET" class="filter-bar">
      ...
      <label for="styleFilter">Cuisine / Style</label>
      <select id="styleFilter" name="style" onchange="this.form.submit()">
        <option value="">All styles</option>
        <option value="Filipino" <?= $styleFilter === 'Filipino' ? 'selected' : '' ?>>Filipino</option>
        <option value="Japanese" <?= $styleFilter === 'Japanese' ? 'selected' : '' ?>>Japanese</option>
        <option value="Mexican" <?= $styleFilter === 'Mexican' ? 'selected' : '' ?>>Mexican</option>
        <option value="Italian" <?= $styleFilter === 'Italian' ? 'selected' : '' ?>>Italian</option>
        <option value="Buffet" <?= $styleFilter === 'Buffet' ? 'selected' : '' ?>>Buffet</option>
      </select>
    </form>
    ...
    <div class="catering-grid">
      ...
      <div class="catering-card">
        <div class="catering-image">
          ...
          <img src="<?= esc(getCateringImage($index)) ?>" alt="<?= esc($service['name']) ?>">
        </div>
...
document.querySelectorAll('.select-btn').forEach(function(btn) {
  btn.addEventListener('click', function() {
    const card = this.closest('.catering-card');
    const name = card.querySelector('h3')?.textContent || 'Catering';
    selectService(name, 'catering');
  });
});
</script>
</body>
