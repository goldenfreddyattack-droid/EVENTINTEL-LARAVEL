...
function getSoundLightsImage($index) {
  $images = [
    'https://images.unsplash.com/photo-1516455581829-8ee6afe3bdf4?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1495092427040-8fcc0ad94d7d?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1518609878373-06d740f60d8b?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1517059224940-d4af9eec41e8?auto=format&fit=crop&w=900&q=80',
  ];
  return $images[$index % count($images)];
}

function getSoundLightsGallery($index) {
  $images = [
    'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?auto=format&fit=crop&w=900&q=80',
    'https://images.unsplash.com/photo-1518972559570-15b1d1262961?auto=format&fit=crop&w=900&q=80',
...
<body>
  ...
  <div class="container">
    ...
    <div class="hero">
      <div>
        <h1><?= $selectedService ? 'Sounds & Lights Details' : 'Select Sounds & Lights' ?></h1>
        <p><?= $selectedService ? 'Review this production package before booking.' : 'Choose the perfect sound system and lighting setup to create the right atmosphere for your event.' ?></p>
      </div>
      ...
      <div class="service-detail">
        <div class="detail-card">
          <div class="detail-image">
            <img src="<?= esc(getSoundLightsGallery($selectedIndex)) ?>" alt="<?= esc($selectedService['name']) ?>">
          </div>
        </div>
        <div class="detail-info">
          <div class="badge">Sounds & Lights</div>
          <h1><?= esc($selectedService['name']) ?></h1>
          <p><?= esc($selectedService['description'] ?? 'Professional sound and lighting services for your event.') ?></p>
          <div class="detail-stats">
            ...
            <div class="detail-stat">
              ...
              <strong>₱<?= number_format($selectedService['price'] ?? 18500) ?></strong>
            </div>
          </div>
          <div class="detail-stats">
            <div class="detail-stat">
              <span>Supplier</span>
              <strong><?= esc($selectedService['supplier_name'] ?? 'Sound & Lights Pro') ?></strong>
            </div>
            <div class="detail-stat">
          ...
          <div class="thumbnail-row">
            <?php for ($i = 0; $i < 4; $i++): ?>
              <div class="thumbnail">
                <img src="<?= esc(getSoundLightsGallery($i)) ?>" alt="Thumbnail <?= $i + 1 ?>">
              </div>
            <?php endfor; ?>
          </div>
        </div>
    ...
    <form method="GET" class="filter-bar">
      ...
      <label for="styleFilter">Production Style</label>
      <select id="styleFilter" name="style" onchange="this.form.submit()">
        <option value="">All styles</option>
        <option value="Concert" <?= $styleFilter === 'Concert' ? 'selected' : '' ?>>Concert</option>
        <option value="Corporate" <?= $styleFilter === 'Corporate' ? 'selected' : '' ?>>Corporate</option>
        <option value="Ambient" <?= $styleFilter === 'Ambient' ? 'selected' : '' ?>>Ambient</option>
        <option value="Stage" <?= $styleFilter === 'Stage' ? 'selected' : '' ?>>Stage</option>
      </select>
    </form>
    ...
    <div class="services-grid">
      ...
      <div class="service-card">
        <div class="service-image">
          ...
          <img src="<?= esc(getSoundLightsImage($index)) ?>" alt="<?= esc($service['name']) ?>">
        </div>
