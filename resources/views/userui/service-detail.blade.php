...
const selection = {type:'serviceSelected', service:@json($serviceKey), [@json($serviceKey)]:@json($serviceRecord->name), price:Number(@json($serviceRecord->price)) || null};
...
document.querySelector('.select-service').addEventListener('click', () => {
    if (@json($serviceKey) === 'venue') {
        const venueSelection = {type:'serviceSelected', service:'venue', venue:@json($serviceRecord->name), price:Number(@json($serviceRecord->price)) || null};
...
document.querySelector('[data-confirm-booking]').addEventListener('click', () => { setBookingModal(bookingConfirmModal, false); bookingSuccessModal.style.display = 'flex'; setBookingModal(bookingSuccessModal, true); });
document.querySelector('[data-back-services]').addEventListener('click', () => {
