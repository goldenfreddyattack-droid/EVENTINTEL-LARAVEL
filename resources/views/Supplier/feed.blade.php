...
@section('styles')
    ...
    <style>
        .newsfeed-content { width:min(980px,100%); margin:0 auto; padding:20px 24px; border:1px solid #ebebeb; border-radius:20px; background:#fff; box-shadow:0 10px 30px rgba(0,0,0,.03); }
        ...
        @forelse ($posts as $post)
            ...
            <article class="post-card" data-post-id="{{ $post->post_id }}">
                ...
                <footer class="post-footer">
                    ...
                    <button type="button" class="share-button" data-url="{{ url('/supplier/feed') }}#post-{{ $post->post_id }}"><i class="fas fa-share-nodes" aria-hidden="true"></i> Share</button>
                </footer>
            </article>
        @empty
