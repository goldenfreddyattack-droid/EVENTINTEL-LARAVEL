...
<style>
    ...
    .global-unread { background: #ef4444; color: #ffffff; border-radius: 999px; font-size: 11px; font-weight: 700; padding: 2px 8px; }
    ...
    .unread-badge { background: #ef4444; color: #ffffff; border-radius: 999px; font-size: 10px; font-weight: 700; padding: 2px 6px; }
    ...
    @media (max-width: 768px) { .messenger-body { grid-template-columns: 1fr; } .messenger-sidebar { display: none; } .messenger-sidebar.show { display: flex; } }
</style>
@endsection
...
<section class="messenger-card">
    ...
    <header class="messenger-header">
        <h2>
            <i class="fas fa-comments text-gold"></i> Messages
            @if($globalUnread > 0)
                <span class="global-unread" id="globalUnreadBadge">{{ $globalUnread }}</span>
            @endif
        </h2>
    </header>
        ...
        <aside class="messenger-sidebar">
            ...
            <div class="thread-list">
                @forelse($threads as $thread)
                    ...
                    <a href="{{ route('supplier.messages', ['event_id' => $thread['event_id'], 'user_id' => $thread['user_id']]) }}" class="thread-item {{ $active ? 'active' : '' }}">
                        <div class="thread-details">
                            <h4>{{ $thread['name'] }}</h4>
                            <p>{{ $thread['event_title'] }} &bull; {{ $thread['event_date'] ?: 'No date' }}</p>
                            <span class="role-pill">{{ $thread['role'] }}</span>
                        </div>
                        @if($thread['unread'] > 0 && !$active)
        ...
        <main class="messenger-chat-box">
            ...
            @else
                ...
                @if(session('message_error'))
                    <div class="chat-alert">{{ session('message_error') }}</div>
                @endif
                @if($errors->any())
                ...
                <div class="chat-stream" id="chatMessages">
                    <div class="msg-bubble received" id="messageStatus">Loading messages...</div>
                </div>

                {{-- Footer Input Row --}}
                <form class="messenger-input-row" method="POST" action="{{ route('supplier.messages.send') }}">
                    @csrf
                    <input type="hidden" name="event_id" value="{{ $selectedEventId }}">
                    <input type="hidden" name="recipient_id" value="{{ $selectedUserId }}">
                    <textarea name="message" placeholder="Type a message..." required>{{ old('message') }}</textarea>
    ...
    (() => {
        ...
        function appendMessages(messages) {
            messages.forEach(message => {
                ...
                item.className = `msg-bubble ${sent ? 'sent' : 'received'}`;
        ...
        async function poll() {
            try {
                const response = await fetch(`${apiUrl}?action=poll&event_id=${eventId}&other_user_id=${otherUserId}`, { headers: { Accept: 'application/json' } });
